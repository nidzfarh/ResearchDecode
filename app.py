...
# CREATE TABLES AGAIN AFTER MODEL
with app.app_context():
    db.create_all()
# PARSER
...
def upload():
    ...
    analysis = Analysis(
        ...
        author=pdf_metadata.get("author","Unknown Author"),pages=str(pdf_metadata.get("pages","-" )))
    ...
    # RECENT ANALYSES
    analyses = Analysis.query.order_by(Analysis.id.desc()).all()
    return render_template("result.html",data=parsed,analyses=analyses,metadata=pdf_metadata,analysis_id=analysis.id)

# VIEW SAVED ANALYSIS
@app.route("/analysis/<int:id>")
def analysis(id):
    analysis = Analysis.query.get_or_404(id)
    parsed = parse_output(analysis.content)
    parsed = markdown_sections(parsed)
    ...
    return render_template("result.html",data=parsed,analyses=analyses,metadata=metadata,analysis_id=analysis.id)
# download pdf
